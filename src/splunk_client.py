...
class SplunkClient:
    ...
    def validate_credentials(self) -> Tuple[bool, str]:
        """Validate Splunk credentials. Returns (is_valid, error_message)."""
        if not self.is_configured():
            return False, "Splunk not configured (missing base_url, username, or password)"
        ...
        try:
            ...
            if "401" in str(e) or "unauthorized" in str(e).lower():
                return False, "Splunk authentication failed (401): Invalid username or password. Check config/.env"
            else:
                return False, f"Credential validation failed: {e}"
            ...
            return False, f"Unexpected error during credential validation: {e}"
    ...
    def _login(self) -> None:
        ...
        try:
            ...
            if login_response.status_code == 401:
                logger.error("Splunk authentication failed (401): Invalid username or password. Please verify credentials in .env file.")
                raise RuntimeError("Splunk authentication failed: Invalid credentials. Check SPLUNK_USERNAME and SPLUNK_PASSWORD in config/.env")
            ...
            logger.error("Failed to connect to Splunk at %s: %s", self.base_url, e)
            raise RuntimeError(f"Cannot reach Splunk instance at {self.base_url}. Check SPLUNK_BASE_URL and network connectivity.")
        except requests.exceptions.Timeout:
            logger.error("Splunk login request timed out")
            raise RuntimeError("Splunk login request timed out. The server may be unreachable or slow.")
        except Exception as e:
            logger.error("Splunk login failed: %s", e)
            raise
    ...
    def list_indexes(self) -> List[str]:
        ...
        try:
            ...
            if e.response.status_code == 401:
                logger.warning("Splunk session expired (401) during list_indexes. Refreshing login and retrying...")
                self._logged_in = False  # Force re-login
                self.session.cookies.clear()  # Clear old cookies
                ...
                try:
                    ...
                    logger.error("Failed to list Splunk indexes after session refresh: %s", retry_error)
                    raise
            else:
                raise
    ...
    def run_search(self, search_query: str) -> List[Dict[str, Any]]:
        ...
        try:
            # Use correct Splunk REST endpoint for search jobs
            response = self.session.post(
                self._proxy_url(f"/services/search/jobs/export"),
                headers=self._headers(),
                data={
                    "search": search_query,
            ...
            # Handle 401 Unauthorized by refreshing session and retrying once
            if e.response.status_code == 401:
                logger.warning("Splunk session expired (401). Refreshing login and retrying...")
                self._logged_in = False  # Force re-login
                self.session.cookies.clear()  # Clear old cookies
                ...
                try:
                    ...
                    logger.error("Splunk search failed after session refresh: %s", retry_error)
                    raise
            else:
                raise
    ...
    def find_relevant_logs(self, incident: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if exc.response.status_code == 401:
                result["error"] = "Splunk authentication failed (401): Invalid credentials or account locked. Check SPLUNK_USERNAME and SPLUNK_PASSWORD in config/.env"
                logger.error("Splunk 401 Unauthorized: %s. Verify credentials in config/.env", exc.response.url)
            else:
                result["error"] = f"Splunk HTTP error {exc.response.status_code}: {exc}"
                logger.warning("Splunk HTTP error: %s", exc)
            result["analysis_context"] = f"Splunk evidence lookup failed: {result['error']}"
            result["report_text"] = self.build_report_text(incident, result)
            return result
    ...
    def extract_identifiers(self, incident: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        labeled_request_ids = {
            match.group(1)
            for match in re.finditer(r"\brequest[_ ]id\s*[:=]\s*(_[0-9]{6,18})\b", text, re.IGNORECASE)
        }
        labeled_quote_numbers = {
            match.group(1)
            for match in re.finditer(r"\bquote[_ ]number\s*[:=]\s*(\d{8})\b", text, re.IGNORECASE)
        }
        labeled_service_names = {
            match.group(1).strip()
            for match in re.finditer(r"\bservice[_ ]name\s*[:=]\s*([A-Za-z][A-Za-z0-9_-]{2,60})", text, re.IGNORECASE)
        }
        labeled_applications = {
            match.group(1).strip().upper()
            for match in re.finditer(r"\bapplication\s*[:=]\s*([A-Za-z][A-Za-z0-9_-]{1,30})", text, re.IGNORECASE)
        }
        labeled_environments = {
            match.group(1).strip().upper()
            for match in re.finditer(r"\benvironment\s*[:=]\s*([A-Za-z][A-Za-z0-9_-]{1,15})", text, re.IGNORECASE)
        }

        request_ids = sorted(set(re.findall(r"\b_[0-9]{6,18}\b", text)).union(labeled_request_ids))
        quote_numbers = sorted(set(re.findall(r"\b\d{8}\b", text)).union(labeled_quote_numbers))

        service_names = sorted(
            set(service_values).union(labeled_service_names).union(
                {
                    token
                    for token in re.findall(r"\b[A-Za-z]+(?:[A-Z][a-z0-9]+)+(?:[A-Z]{2,})?\b", text)
        ...
        application_tokens = sorted(
            set(application_values).union(labeled_applications).union(
                {
                    token.upper()
        ...
        environments = sorted(
            set(environment_values).union(labeled_environments).union(
                {
                    token.upper()
    ...
    def build_analysis_context(self, evidence: Dict[str, Any]) -> str:
        ...
        if evidence.get("error"):
            return (
                "Splunk evidence was unavailable because the lookup failed. "
                "Treat this as missing evidence only, not as the incident root cause, unless the ticket itself mentions Splunk. "
                f"Lookup error: {evidence['error']}"
            )
