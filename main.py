...
def main():
    ...
    try:
        ...
        if splunk_client:
            ...
            if is_valid:
                logger.info("✓ Splunk credentials validated successfully")
            else:
                logger.warning("✗ Splunk credential validation failed: %s", error_msg)
                logger.warning("⚠ Splunk evidence retrieval will be disabled due to authentication issues")
                splunk_client = None
        else:
